...
import pretorchedx.pretorched.models.r2plus1d as r2plus1d
...
class ExtractR21D(BaseExtractor):
    ...
    def __init__(self, args) -> None:
        ...
        r21d_model_cfgs = {
            ...
            'r2plus1d_34_32_ig65m_ft_kinetics': {
                ...
                'stack_size': 32, 'step_size': 32, 'num_classes': 400, 'dataset': 'kinetics' #### ccd 겹치려면 step_size: 32 -> 18
            },
    ...
    def load_model(self) -> Dict[str, torch.nn.Module]:
        ...
        if self.model_name == 'r2plus1d_18_16_kinetics':
            ...
            model = r2plus1d34(num_classes=self.model_def['num_classes'])
    ...
    def maybe_show_pred(self, visual_feats: torch.Tensor, start_idx: int, end_idx: int, video_path): #### video_path 추가함
        if self.show_pred:
            logits = self.name2module['class_head'](visual_feats)
            print(f'At frames ({start_idx}, {end_idx})')
            show_predictions_on_dataset(logits, self.model_def['dataset'], start_idx/32, video_path) #### start_idx/32, video_path 추가함

...
def show_predictions_on_dataset(logits: torch.FloatTensor, dataset: Union[str, List], seg_id: int, video_path): #### segment_id 추가함 (0부터 시작), video_path 추가함
    ...
    # 저장할 파일 생성 및 열기
    softmax_file = open('../r21d_tadaconv_new_kin_softmax.csv', 'a')
    ...
    # 비디오 경로와 softmax 배열을 csv 파일에 작성 
    soft_list = softmaxes.tolist()
    tmp_list = [] # soft_list와 합치려면 list형식이어야 해서 만들어줌
    tmp_list.append(video_path)
    tmp_list.append(int(seg_id))
    writer.writerow(tmp_list + soft_list) # video_path 추가해야 함!!
    softmax_file.close()
